...
export function SkillDetailPanel({
  ...
  if (!node) {
    return (
      <aside className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="text-sm font-semibold text-slate-950">节点详情</h2>
  ...
  return (
    <aside className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      <p className="text-sm font-semibold text-slate-950">节点详情</p>
      ...
      <dl className="mt-4 grid gap-3 text-sm">
        <div>
          <dt className="text-slate-500">建议阶段</dt>
      ...
      <div className="mt-6">
        <p className="text-sm font-medium text-slate-950">标记状态</p>
          ...
          {statusOptions.map((option) => {
            ...
            return (
              <button
                ...
                className={`flex items-center justify-between rounded-md border px-3 py-2 text-sm font-medium transition hover:bg-slate-50 ${
